...
def selection_sort (lista:list) -> list:
    ...
    for i in range(len(lista)-1): # cuando ya se acomodó hasta el anteúltimo número, el último que queda al final ya queda ordenado solo. Luego, este bucle puede frenar uno antes.
        ...
        # Antes del intercambio verifico si el índice del mínimo es diferente al índice actual (i) para evitar intercambios innecesarios
        if indice_minimo != i:    
            # Creo un elemento auxiliar para hacer el intercambio
            temp = lista[i]
            lista[i] = lista[indice_minimo] # Asigno el valor del mínimo a la posición actual (i)
            lista[indice_minimo] = temp # Asigno el valor que estaba en la posición actual (i) a la posición del mínimo (indice_minimo)
